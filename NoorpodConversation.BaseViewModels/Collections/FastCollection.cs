...
namespace NoorpodConversation.BaseViewModels.Collections
{
    public static class FastCollectionRemoved
    {
        public static Action<object> RemovedForValidationAction { get; set; }
    }
    ...
    public class FastCollection<T> : IList<T>, ICollection<T>, IEnumerable<T>, IList, ICollection, IEnumerable, INotifyCollectionChanged, INotifyPropertyChanged
    {
        ...
        public void ActionDispatcher(Action action)
        {
            if (_dispatcherThread == null)
                ...
                ApplicationHelper.Current.EnterDispatcherThreadActionForCollections(action, _dispatcherThread);
        }
        ...
        public void Clear()
        {
            lock (lockobj)
            {
                ...
                {
                    FastCollectionRemoved.RemovedForValidationAction?.Invoke(item);
                }
                _bindList.Clear();
                OnCollectionChanged();
            }
        ...
        public void OnCollectionChanged(NotifyCollectionChangedAction mode = NotifyCollectionChangedAction.Reset)
        {
            ActionDispatcher(() =>
            {
                ...
                {
                    AutoLogger.LogError(ex, "Fast Collection OnCollectionChanged");
                }
            });
        }
